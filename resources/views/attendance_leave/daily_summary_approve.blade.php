            ...
            <div id="kt_app_toolbar_container" class="app-container">
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">
                        Daily Summary Approve (OT &amp; Production)
                    </h1>
                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                        <li class="breadcrumb-item text-muted">Attendance & Leave</li>
                        <li class="breadcrumb-separator"></li>
                        <li class="breadcrumb-item text-gray-700">Daily Summary Approve</li>
                    </ul>
                </div>
            </div>
        </div>
                    ...
                    <div class="card-body p-0 p-2">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3 px-2">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="selectAllOT" />
                                <label class="form-check-label fw-semibold" for="selectAllOT">Select All Records</label>
                            </div>
                            <button type="button" class="btn btn-primary btn-sm px-4" id="approveOTBtn">
                                <i class="fas fa-check-circle me-1"></i> Approve OT
                            </button>
                        </div>

                        {{-- OT DataTable --}}
                        <div class="table-responsive">
                            <table class="table align-middle table-row-dashed fs-6 gy-5" id="otTable">
                                <thead>
                ...
                <div class="card">
                    <div class="card-body p-0 p-2">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3 mt-3 px-2">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="selectAllProduction" />
                                <label class="form-check-label fw-semibold" for="selectAllProduction">Select All Records</label>
                            </div>
                        ...
                        <div class="table-responsive">
                            <table class="table align-middle table-row-dashed fs-6 gy-5" id="productionTable">
                                <thead>
                                    <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                                        <th><input type="checkbox" class="form-check-input" id="productionCheckAll" /></th>
                                        <th>EMPLOYEE ID</th>
                                        <th>EMPLOYEE</th>
                                        <th>DATE</th>
                                        <th>TARGET</th>
...
<script>
    $(document).ready(function () {
        ...
        var otTable = $('#otTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('daily_summary_approve') }}",
                type: 'GET',
                data: function (d) {
            ...
            columns: [
                {
                    ...
                    width: '40px',
                    render: function (data, type, row) {
                        return `<input type="checkbox" class="form-check-input production-row-check" value="${row.id}" />`;
                    }
                },
                { data: 'emp_id',       name: 'emp_id' },
                { data: 'employee',     name: 'employee' },
                ...
                { data: 'day_produce',  name: 'day_produce' },
                { data: 'plus_minus',   name: 'plus_minus' },
        ...
        $('#selectAllOT, #otCheckAll').on('change', function () {
            var checked = $(this).prop('checked');
            $('#selectAllOT, #otCheckAll').prop('checked', checked);
            $('#otTable tbody .ot-row-check').prop('checked', checked);
        });

        $('#selectAllProduction, #productionCheckAll').on('change', function () {
            var checked = $(this).prop('checked');
            $('#selectAllProduction, #productionCheckAll').prop('checked', checked);
            $('#productionTable tbody .production-row-check').prop('checked', checked);
        });
        ...
        $('#approveOTBtn').on('click', function () {
            var selectedIds = [];
            $('#otTable tbody .ot-row-check:checked').each(function () {
            ...
            if (selectedIds.length === 0) {
                Swal.fire({
                    ...
                    text: 'Please select at least one OT record to approve.'
                });
                return;
            }
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        error: function () {
                            Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to approve OT records.' });
                        }
                    });
                }
            });
        ...
        $('#approveAllBtn').on('click', function () {
            var selectedIds = [];
            $('#productionTable tbody .production-row-check:checked').each(function () {
                selectedIds.push($(this).val());
            });
            ...
            if (selectedIds.length === 0) {
                Swal.fire({
                    icon: 'warning',
                    title: 'No Records Selected',
                    text: 'Please select at least one production record to approve.'
                });
                return;
            }
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Approved!',
                                text: response.message ?? 'Production records approved successfully.',
                                timer: 2000,
                                showConfirmButton: false
                            });
                        ...
                        error: function () {
                            Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to approve production records.' });
                        }
                    });
                }
            });
