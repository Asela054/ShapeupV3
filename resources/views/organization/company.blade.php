	...
	<script>
		...
		$(document).ready(function () {
			...
			function clearFormErrors() {
				...
				$('#companyForm .form-select').removeClass('is-invalid');
			}
			...
			$('.backToCompany').on('click', function () {
				localStorage.removeItem('org_active_view');
				localStorage.removeItem('org_company_id');
				localStorage.removeItem('org_company_name');
				if (window.location.hash) {
					history.pushState(null, null, window.location.pathname + window.location.search.replace(/[\?&]_view=[^&]+/, ''));
				}
						...
						render: function (data, type, row) {
							...
							return `
								<div class="d-inline-flex align-items-center justify-content-end">
									<button type="button" class="btn btn-sm me-1 text-white viewDepartments d-inline-flex align-items-center justify-content-center" data-id="${row.id}" data-name="${escapedName}" style="background-color: #00c5ce; width: 32px; height: 32px; border-radius: 6px;" title="Department">
										<i class="fa-solid fa-building text-white fs-6"></i>
									</button>
									<button type="button" class="btn btn-sm me-1 text-white viewBranches d-inline-flex align-items-center justify-content-center" data-id="${row.id}" data-name="${escapedName}" style="background-color: #7000da; width: 32px; height: 32px; border-radius: 6px;" title="Branch">
