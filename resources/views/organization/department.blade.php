				...
				<form id="departmentForm" method="POST" action="">
					...
					<div class="row g-4">
						...
						<div class="col-md-12">
							...
							<select name="dep_head_emp_id" id="dep_head_emp_id" class="form-select">
								<option value="">Select Head</option>
								@if(isset($employees))
									@foreach($employees as $emp)
										<option value="{{ $emp->id }}">{{ $emp->calling_name ?? $emp->full_name }}</option>
									@endforeach
								@endif
							</select>
							<span class="text-danger" id="error_dep_head_emp_id"></span>
						</div>
					</div>
...
<script>
	$(document).ready(function () {
		...
		function showDepartmentView(companyId, companyName) {
			...
			$('#breadcrumbActive').text(selectedCompanyName ? `Department (${selectedCompanyName})` : 'Department');
			$('#departmentViewSubtitle').text(selectedCompanyName ? `- ${selectedCompanyName}` : '');
		...
		$(document).on('click', '.editDepartment', function (e) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					$('#dep_head_emp_id').val(data.dep_head_emp_id || '');
